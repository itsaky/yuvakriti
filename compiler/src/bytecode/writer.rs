...
impl LoopContext {
    ...
    fn matches_label(&mut self, label: Option<&IdentifierExpr>) -> bool {
        ...
        if self.label.as_ref().map(|l| l == _label).unwrap_or(false) {
            return true;
        }
...
impl ASTVisitor<CodeGenContext<'_>, ()> for CodeGen<'_> {
    ...
    fn visit_block_stmt(
        ...
        self.default_visit_block_stmt(
            block_stmt,
            &mut CodeGenContext::with_scope(scope, ctx.loops.as_mut()),
        )
    }
    ...
    fn visit_for_stmt(
        ...
        ctx.push_loop(
            cp,
            LoopType::For,
            for_stmt.label.as_ref().map(|l| l.name.clone()),
        );
